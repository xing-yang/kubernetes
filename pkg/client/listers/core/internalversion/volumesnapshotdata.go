...
type VolumeSnapshotDataLister interface {
	...
	List(selector labels.Selector) (ret []*core.VolumeSnapshotData, err error)
	// Get retrieves the VolumeSnapshotData from the index for a given name.
	Get(name string) (*core.VolumeSnapshotData, error)
...
// List lists all VolumeSnapshotDatas in the indexer.
func (s *volumeSnapshotDataLister) List(selector labels.Selector) (ret []*core.VolumeSnapshotData, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*core.VolumeSnapshotData))
	})
	return ret, err
}

// Get retrieves the VolumeSnapshotData from the index for a given name.
func (s *volumeSnapshotDataLister) Get(name string) (*core.VolumeSnapshotData, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(core.Resource("volumesnapshotdata"), name)
	}
	return obj.(*core.VolumeSnapshotData), nil
}
